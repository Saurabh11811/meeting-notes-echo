  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] px-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl bg-echo-surface border border-echo-border rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-1 px-3 py-2 border-b border-echo-border bg-echo-surface-2">
          {filters.map((f) => (
            <button
              ...
              className={`px-2.5 py-1 rounded text-[14px] ${filter === f.id ? "bg-echo-accent-bg text-echo-accent-fg" : "text-echo-text-muted hover:bg-echo-surface-hover"}`}
            >{f.label}</button>
          ))}
          <span className="ml-auto text-[14px] text-echo-text-faint">{results.length} results</span>
        </div>

        <ul className="max-h-[420px] overflow-y-auto py-1">
          {results.length === 0 && (
            <li className="px-6 py-10 text-center text-[14px] text-echo-text-muted">No saved meetings found.</li>
          )}
          {results.map((r, i) => {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-[12px] uppercase tracking-wide text-echo-text-faint">{meta.label}</span>
                      <span className="text-[12px] text-echo-text-faint">·</span>
                      <span className="text-[13px] text-echo-text-muted truncate">{r.meeting} · {r.date}{r.timestamp ? ` · ${r.timestamp}` : ""}</span>
                    </div>
                    <div className="text-[15px] text-echo-text truncate mt-0.5" style={{ fontWeight: 500 }}>{r.title}</div>
                    <div className="text-[13px] text-echo-text-muted truncate">{r.context}</div>
                  </div>
                </button>
              </li>
            );
        ...
        <div className="px-4 py-2 border-t border-echo-border bg-echo-surface-2 flex items-center gap-4 text-[12px] text-echo-text-faint">
