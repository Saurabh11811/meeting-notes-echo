  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        actions={<button className="h-9 px-3 rounded-md bg-echo-text text-echo-surface text-[14px] inline-flex items-center gap-1.5"><Plus size={13} />New template</button>}
      />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <div className="text-[14px] text-echo-text" style={{ fontWeight: 600 }}>{t.name}</div>
                      {t.isDefault && <span className="text-[14px] px-1.5 py-0.5 rounded bg-echo-accent-bg text-echo-accent-fg">Default</span>}
                      {t.locked && <Lock size={11} className="text-echo-text-faint" />}
                    </div>
                    <p className="text-[14px] text-echo-text-muted mt-1">{t.desc}</p>
                    <div className="text-[14px] text-echo-text-faint mt-2">Updated {t.updated}</div>
                  </div>
                </div>
              </button>
            );
        ...
        <aside className="bg-echo-surface border border-echo-border rounded-lg overflow-hidden h-fit">
          <div className="px-5 py-4 border-b border-echo-border flex items-start gap-3">
            ...
            <div className="flex-1">
              <h2 className="text-[14px] text-echo-text" style={{ fontWeight: 600 }}>{current.name}</h2>
              <p className="text-[14px] text-echo-text-muted">{current.desc}</p>
            </div>
          </div>

          <div className="p-5 space-y-4 text-[14px]">
            <div>
              <h3 className="text-[14px] text-echo-text-muted uppercase tracking-wide mb-2">Sections included</h3>
              <div className="flex flex-wrap gap-1.5">
                {["Executive Summary", "Decisions", "Action Items", "Risks", "Discussion Points", "Next Steps"].map((s) => (
                  <span key={s} className="px-2 py-0.5 rounded bg-echo-surface-2 text-echo-text-muted text-[14px]">{s}</span>
                ))}
              </div>
            </div>

            <div>
              <h3 className="text-[14px] text-echo-text-muted uppercase tracking-wide mb-2">Output preview</h3>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button className="h-9 rounded-md bg-echo-accent text-white text-[14px] inline-flex items-center justify-center gap-1.5"><Star size={12} />Use as default</button>
              <button className="h-9 rounded-md border border-echo-border bg-echo-surface text-echo-text text-[14px] inline-flex items-center justify-center gap-1.5"><Copy size={12} />Duplicate</button>
              <button className="h-9 rounded-md border border-echo-border bg-echo-surface text-echo-text text-[14px] inline-flex items-center justify-center gap-1.5"><Pencil size={12} />Edit</button>
              <button className="h-9 rounded-md border border-echo-border bg-echo-surface text-echo-text text-[14px] inline-flex items-center justify-center gap-1.5"><FlaskConical size={12} />Test with transcript</button>
            </div>

            <div className="text-[14px] text-echo-text-muted pt-2 border-t border-echo-border">
